...
export default function Login() {
  ...
  window.location.href = '/dashboard' // এটা লিখেন
}
  console.log('Button Clicked!', email, password) // এটা Add
  
  if(email && password) {
    localStorage.setItem('token', 'dummy-token-123')
    console.log('Token Set') // এটা Add
    navigate('/dashboard')
    console.log('Navigating...') // এটা Add
  }
};
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            ...
            <input
              ...
              required
              className="w-full p-3 transition border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 font-semibold text-white transition duration-200 bg-blue-600 rounded-lg shadow-md hover:bg-blue-700 disabled:bg-blue-400"
          >
